import { hasLike } from "@/actions/post";
    ...
    <div className="py-4">
      {post.thumbnailURL && (
        <div className="aspect-video relative w-full overflow-hidden rounded-lg mb-4">
          <Image sizes="800px" src={post.thumbnailURL} className="object-cover" fill alt="" />
        </div>
      )}
      <div className="flex items-center mb-2">
        <div className="w-12 h-12 rounded-full grid place-content-center overflow-hidden bg-gray-100 mr-3 relative">
          {post.author.profileImageURL ? (
            <Image sizes="48px" src={post.author.profileImageURL} className="object-cover" fill alt="" />
          ) : (
            <User size={24} className="text-gray-400" />
          )}
        </div>
        <div className="flex-1">
          <p className="mb-2 leading-none text-gray-700 font-semibold">{post.author.name}</p>
